...
export function setUserPosition(pos) {
    if (!map)
        return;
    if (!userMarkerEl) {
...
export function updateStationMarkers(results) {
    if (!map)
        return;
...
export function showRouteLine(from, to) {
    if (!map)
        return;
    removeRouteLine();
    const id = 'route-line';
...
export function toggleHeatmap(results, show) {
    if (!map)
        return;
