...
let panel = null;
export function initSmartCalculator() {
...
function renderCalcBody() {
    ...
    <div class="space-y-4">
      ...
      <div class="flex gap-2">
        ${['e5', 'e10', 'diesel'].map(ft => `<button data-calcfuel="${ft}" class="flex-1 py-2 rounded-lg text-xs font-medium transition-colors ${ft === fuelType ? 'bg-[var(--fuel-accent)] text-white' : 'bg-[var(--fuel-surface-2)] text-[var(--fuel-text-muted)] hover:text-[var(--fuel-text)]'}">${ft.toUpperCase()}</button>`).join('')}
      </div>
    </div>
  ...
  `;
    body.querySelector('#calc-volume')?.addEventListener('input', (e) => {
        store.setUserProfile({ tankVolume: parseFloat(e.target.value) });
    });
    body.querySelector('#calc-consumption')?.addEventListener('input', (e) => {
        store.setUserProfile({ consumption: parseFloat(e.target.value) });
    });
    body.querySelector('#calc-fill')?.addEventListener('input', (e) => {
        store.setUserProfile({ currentFillPercent: parseInt(e.target.value) });
    });
    body.querySelectorAll('[data-calcfuel]').forEach(btn => {
        btn.addEventListener('click', () => {
            const ft = btn.getAttribute('data-calcfuel');
