import type { TKStation, UserProfile, SmartResult } from '../types';

export function calculateSmartResults(
  stations: TKStation[],
  ...
  const results: SmartResult[] = openStations.map(station => {
  ...
  if (bestValueIdx >= 0) results[bestValueIdx].recommendation = 'BEST_VALUE';
  if (cheapestIdx >= 0 && cheapestIdx !== bestValueIdx) results[cheapestIdx].recommendation = 'CHEAPEST';
...
function getPrice(station: TKStation, fuelType: string): number | null {
  switch (fuelType) {
